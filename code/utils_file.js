...
// - desktop: simulating click on hidden `input` HTML element, created using given `options`
...
// @event loaded: Event
// Fired after all files are processed (either with success or with error).
window.loadFiles = typeof android === 'undefined' || !android.getFileRequestUrlPrefix
  ? L.FileListLoader.loadFiles.bind(L.FileListLoader)
  : function () { // compatibility shim for IITCm
    var loader = new L.Evented();
    window.requestFile(function (filename,text) {
      ...
      try {
        this.fire('init', data);
        data.reader = {result: text};
        this.fire('load',data);
      } catch (e) {
        this.fire('init:error', data);
      } finally {
        this.fire('loaded');
      }
    }.bind(loader));
