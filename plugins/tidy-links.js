...
tidyLinks.draw = function (locations, layer) {
  var triangles = tidyLinks.Delaunay.triangulate(
...
function setup() {
  ...
  tidyLinks.layer = L.layerGroup([])
    .on('add', function () {
      ...
      if (window.plugin.drawTools && window.plugin.drawTools.filterEvents) {
        window.plugin.drawTools.filterEvents.on('changed', tidyLinks.update);
      }
    })
    .on('remove', function () {
      window.removeHook('mapDataRefreshEnd', tidyLinks.update);
      if (window.plugin.drawTools && window.plugin.drawTools.filterEvents) {
        window.plugin.drawTools.filterEvents.off('changed', tidyLinks.update);
      }
    })
  ...
  tidyLinks.layer.getCenter = function () {
