...
var cache = {};
function getLayer(e) {
  var mapped = getMapping(e.name) || (!miniMap.clonebasemap && miniMap.layers.default);
  if (mapped) {
    if (mapped instanceof L.Layer) {
      return mapped;
    } // no need to cache
    e = { name: mapped };
  }
...
function setup() {
  ...
  map = window.map;
  layerChooser = window.layerChooser;
  ...
  map.on('baselayerchange', function (e) {
    if (!map.hasLayer(e.layer)) {
      return;
    } // event may come not from main map (todo: debug)
  ...
  if (!miniMap.options.mapOptions.attributionControl) {
    // hide attribution
    $('<style>')
      .html(
