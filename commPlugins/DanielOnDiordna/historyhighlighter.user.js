...
// @name           History Highlighters
// @category       Highlighter
// @version        0.0.1.20210724.002500
// @updateURL      https://raw.githubusercontent.com/IITC-CE/Community-plugins/master/dist/DanielOnDiordna/historyhighlighter.meta.js
// @downloadURL    https://raw.githubusercontent.com/IITC-CE/Community-plugins/master/dist/DanielOnDiordna/historyhighlighter.user.js
...
// @id             historyhighlighter@DanielOnDiordna
...
`;
    ...
    self.hideportal = function(portal) {
        portal.setStyle({opacity:self.settings.hideOpacity,fillOpacity:self.settings.hideOpacity});
    };
    ...
    self.portalwithouthistory = function(portal) {
        if (self.settings.markportalsmissinghistory)
            portal.setStyle({fillColor:self.settings.colormissinghistory,dashArray:"1,4"});
    };
    ...
    self.highlighterHideVisitedportals = function(data) {
        if (!data.portal.options.data.history || data.portal.options.data.history._raw == undefined)
            ...
            self.colorportal(data.portal,self.settings.color.nevervisited,self.settings.coloralpha.nevervisited);
    };
    ...
    self.highlighterHideCapturedPortals = function(data) {
        if (!data.portal.options.data.history || data.portal.options.data.history._raw == undefined)
            ...
            self.hideportal(data.portal);
        else if (data.portal.options.team == window.TEAM_ENL && self.settings.colorenabled.nevercapturedenl)
            self.colorportal(data.portal,self.settings.color.nevercapturedenl,self.settings.coloralpha.nevercapturedenl);
        else if (data.portal.options.team == window.TEAM_RES && self.settings.colorenabled.nevercapturedres)
            self.colorportal(data.portal,self.settings.color.nevercapturedres,self.settings.coloralpha.nevercapturedres);
        else if (data.portal.options.team == window.TEAM_NONE && self.settings.colorenabled.nevercapturedneutral)
            self.colorportal(data.portal,self.settings.color.nevercapturedneutral,self.settings.coloralpha.nevercapturedneutral);
    };

    self.highlighterHideCapturedAndENLPortals = function(data) {
        if (!data.portal.options.data.history || data.portal.options.data.history._raw == undefined)
            self.portalwithouthistory(data.portal);
        else if (data.portal.options.data.history.captured || data.portal.options.team == window.TEAM_ENL)
            self.hideportal(data.portal);
        else if (data.portal.options.team == window.TEAM_RES && self.settings.colorenabled.nevercapturedres)
            self.colorportal(data.portal,self.settings.color.nevercapturedres,self.settings.coloralpha.nevercapturedres);
        else if (data.portal.options.team == window.TEAM_NONE && self.settings.colorenabled.nevercapturedneutral)
    ...
    self.highlighterHideCapturedAndRESPortals = function(data) {
        if (!data.portal.options.data.history || data.portal.options.data.history._raw == undefined)
            self.portalwithouthistory(data.portal);
        else if (data.portal.options.data.history.captured || data.portal.options.team == window.TEAM_RES)
            self.hideportal(data.portal);
        else if (data.portal.options.team == window.TEAM_ENL && self.settings.colorenabled.nevercapturedenl)
            self.colorportal(data.portal,self.settings.color.nevercapturedenl,self.settings.coloralpha.nevercapturedenl);
        else if (data.portal.options.team == window.TEAM_NONE && self.settings.colorenabled.nevercapturedneutral)
    ...
    self.highlighterHideScoutControlledportals = function(data) {
        if (!data.portal.options.data.history || data.portal.options.data.history._raw == undefined)
            ...
            self.colorportal(data.portal,self.settings.color.neverscoutControlled,self.settings.coloralpha.neverscoutControlled);
    };
    ...
    self.menu = function() {
        var html = '<div>';
        html +=
            '<input type="text" id="hideOpacityspinner" name="hideOpacity" value="' + (100 - self.settings.hideOpacity * 100) + '" readonly size="5" style="text-align: center"> Hide portals with history<br />';
        html +=
            '<input type="color" id="' + self.id + '_colormissinghistory" /> ';
        html +=
            '<input type="checkbox" onclick="' + self.namespace + 'settings.markportalsmissinghistory = this.checked; ' + self.namespace + 'storesettings(); window.resetHighlightedPortals();" id="markportalsmissinghistorytoggle"' + (self.settings.markportalsmissinghistory?' checked':'') + '>' +
        ...
        for (const id in self.settings.color) {
            html +=
                '<input type="color" id="' + self.id + '_color_' + id + '" /> ' +
                '<input type="checkbox" onclick="' + self.namespace + 'togglecolor(\'' + id + '\',this.checked);" id="' + self.id + '_toggle_' + id + '"' + (self.settings.colorenabled[id]?' checked':'') + '>' +
                '<label for="' + self.id + '_toggle_' + id + '" style="user-select: none">' + self.colordescription[id] + '</label><br />';
        }
        html +=
        ...
        $('#' + self.id + '_colormissinghistory').spectrum($.extend({}, self.colorpickeroptions, {
        ...
        for (const id in self.settings.color) {
            $('#' + self.id + '_color_' + id).spectrum($.extend({}, self.colorpickeroptions, {
                change: function(color) {
                ...
                color: tinycolor(self.settings.color[id]).setAlpha(self.settings.coloralpha[id])
            }));
        }
    ...
    self.setup = function() {
        ...
        if (!window._current_highlighter) window._current_highlighter = window._no_highlighter; // prevent first highlighter to be enabled by default

        // highlighters are placed in alphabetical order when displayed in the menu:
        window.addPortalHighlighter('Hide All portals', self.highlighterHideportals);
        window.addPortalHighlighter('Hide Captured Portals', self.highlighterHideCapturedPortals);
        window.addPortalHighlighter('Hide Captured + all ENL Portals', self.highlighterHideCapturedAndENLPortals);
        window.addPortalHighlighter('Hide Captured + all RES Portals', self.highlighterHideCapturedAndRESPortals);
        window.addPortalHighlighter('Hide Scout Controlled portals', self.highlighterHideScoutControlledportals);
        window.addPortalHighlighter('Hide Visited portals', self.highlighterHideVisitedportals);
