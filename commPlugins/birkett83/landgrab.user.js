...
// @updateURL       https://raw.githubusercontent.com/IITC-CE/Community-plugins/master/dist/birkett83/landgrab.meta.js
// @downloadURL     https://raw.githubusercontent.com/IITC-CE/Community-plugins/master/dist/birkett83/landgrab.user.js
// @issueTracker    https://github.com/birkett83/landgrab/issues
// @homepageURL     https://birkett83.github.io/landgrab/
...
DSF.prototype.find = function(idx) {
    var prevIdx = idx;
    var nextIdx = this._rep[idx];
    while (nextIdx != undefined) {
        // Path compression
        this._rep[prevIdx] = nextIdx;
        prevIdx = idx;
        idx = nextIdx;
        nextIdx = this._rep[idx];
    }
    return idx;
}

DSF.prototype.rank = function(idx) {
    // Initialising the scores/ranks is a pain because we don't know how big the array will be.
    // Instead we just use (score || 0).
    return (this._rank[idx] || 0);
}

DSF.prototype.score = function(idx) {
    let rep = this.find(idx);
    return (this._score[rep] || 0);
}

DSF.prototype.addScore = function(idx, score) {
    let rep = this.find(idx);
    this._score[rep] = (this._score[rep] || 0) + score;
}

DSF.prototype.union = function (i1, i2) {
    var r1 = this.find(i1);
    var r2 = this.find(i2);
    if (r1 == r2) {return};
    let score = this.score(r1) + this.score(r2);
    delete this._score[r1];
    delete this._score[r2];

    if (this.rank(r1) > this.rank(r2)) {
        this._rep[r2] = r1;
        this._score[r1] = score;
    } else if (this.rank(r2) > this.rank(r1)) {
        this._rep[r1] = r2;
        this._score[r2] = score;
    } else {
        this._rank[r1] = this.rank(r1) + 1;
        this._rep[r2] = r1;
        this._score[r1] = score;
    }
}
